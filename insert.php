...
switch ($action) {
    ...
    case 'register_parent':
        ...
        // Build student array — accept either indexed POST array or keyed array
        $students = [];
        ...
        try {
            ...
            $stmt = $pdb->prepare(
                'INSERT INTO parents
                    (first_name, last_name, phone, email, address, relationship,
                     national_id_type, national_id_no, photo_path)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)'
            );
            $stmt->execute([
            ...
            $stmtStudent = $pdb->prepare(
                'INSERT INTO students
                    (parent_id, first_name, last_name, student_class, house,
                     nhis_id, date_of_birth, gender, student_id_no, medical_condition, photo_path)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
            );
            ...
            for ($i = 0; $i < $wardCount; $i++) {
                ...
                // Link student to the registering parent in the junction table
    ...
    case 'create_event':
        ...
        jsonResponse(true, 'Event created successfully!', ['event_id' => (int)$db->lastInsertId()]);
        break;
    ...
    // ──────────────────────────────────────────────────────────
    // 7. LINK EXISTING PARENT TO EXISTING STUDENT
    // ──────────────────────────────────────────────────────────
    case 'link_parent_student':
        ...
        $pc->execute([$parentId]);
        if (!$pc->fetch()) jsonResponse(false, 'Parent not found.');

        $sc = $pdb->prepare('SELECT id FROM students WHERE id=? AND is_linked_copy=0 LIMIT 1');
        $sc->execute([$studentId]);
        if (!$sc->fetch()) jsonResponse(false, 'Student not found.');
    ...
    case 'submit_exeat':
        ...
        // Block duplicate pending exeat for the same student
        ...
        $stmt = $pdb->prepare(
            'INSERT INTO exeat_requests
                (student_id, parent_id, reason, departure_date, departure_time, expected_return)
             VALUES (?, ?, ?, ?, ?, ?)'
        );
    ...
    // ──────────────────────────────────────────────────────────
    // 9. REVIEW EXEAT (domestic affairs / admin)
    // ──────────────────────────────────────────────────────────
    case 'review_exeat':
        if (empty($_SESSION['admin_id'])) jsonResponse(false, 'Unauthorised.');
    ...
    // ──────────────────────────────────────────────────────────
    // 10. CHANGE CREDENTIALS (any staff user)
    // ──────────────────────────────────────────────────────────
