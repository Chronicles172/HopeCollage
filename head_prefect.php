...
// Fetch ALL Head Prefect candidates
$headPrefectResult = mysqli_query($connect, "SELECT * FROM candidatedetails WHERE LOWER(TRIM(position)) = 'head prefect' ORDER BY candidateid");

$candidates = [];
while ($row = mysqli_fetch_assoc($headPrefectResult)) {
    $candidates[] = $row;
}
...
<html>
<head>
<title>Head Prefect Voting | Sonrise Christian High School</title>
...
<style>
.candidate-image { width:100%; height:350px !important; background-color:#f8f9fa; overflow:hidden; position:relative; border-bottom:3px solid #e0e0e0; }
.candidate-image img { width:100%; height:100%; object-fit:contain !important; object-position:center; padding:10px; }
.candidate-container { display:grid; grid-template-columns:repeat(auto-fit, minmax(280px,1fr)); gap:25px; margin-bottom:20px; }
.candidate-card { background:white; border-radius:12px; border:1px solid #e0e0e0; overflow:hidden; transition:all 0.3s ease; position:relative; box-shadow:0 2px 8px rgba(0,0,0,0.1); display:flex; flex-direction:column; }
.candidate-card:hover { transform:translateY(-8px); box-shadow:0 12px 24px rgba(0,0,0,0.15); border-color:#8A2BE2; }
.candidate-card.selected { border:3px solid #f093fb; box-shadow:0 0 20px rgba(240,147,251,0.5); transform:translateY(-5px); }
...
.candidate-gender { margin-bottom:15px; font-size:1rem; font-weight:500; }
.candidate-gender.male { color:#1976d2; } .candidate-gender.female { color:#c2185b; }
.candidate-gender i { font-size:1.2rem; margin-right:5px; }
.select-btn { display:block; width:100%; background:#f0f0f0; color:#333; border:2px solid #7b1fa2; padding:12px; border-radius:8px; cursor:pointer; transition:all 0.3s; font-weight:600; font-size:1rem; margin-top:10px; }
.select-btn:hover { background:linear-gradient(135deg,#f8bbd0,#bbdefb); transform:scale(1.02); }
.custom-radio:checked + .select-btn { background:linear-gradient(135deg,#7209b7,#8A2BE2); color:white; border-color:#f093fb; }
...
.single-candidate-badge { display:inline-block; background:#fff3e0; color:#e65100; border:1px solid #ffcc80; padding:3px 10px; border-radius:12px; font-size:0.78rem; font-weight:600; margin-bottom:10px; }
.header { background:linear-gradient(135deg,#7209b7,#8A2BE2); color:white; padding:50px 20px; text-align:center; box-shadow:0 4px 15px rgba(0,0,0,0.2); }
.unisex-badge { display:inline-block; background:rgba(255,255,255,0.3); padding:8px 20px; border-radius:25px; font-size:0.9rem; margin-top:10px; font-weight:600; letter-spacing:1px; }
.position-heading { background:linear-gradient(135deg,#191970,#3a3a8c); color:white; padding:15px 25px; border-radius:10px; margin-bottom:30px; font-size:1.5rem; font-weight:bold; text-align:center; box-shadow:0 4px 15px rgba(240,147,251,0.3); }
.position-heading i { margin-right:10px; }
...
@media screen and (max-width:768px) {
  ...
  .toggle-btn { left:70px; } .candidate-container { grid-template-columns:1fr; } .candidate-image { height:350px !important; }
}
...
<div class="main">
  <div class="header">
    ...
    <p>Head Prefect Electronic Voting</p>
    <span class="unisex-badge"><i class="fas fa-crown"></i> UNISEX POSITION - ONE WINNER</span>
  </div>

  <?php if (empty($candidates)): ?>
    <div class="alert alert-info text-center"><i class="fas fa-info-circle mr-2"></i><strong>No Head Prefect candidates available at this time.</strong></div>
  <?php else: ?>
    <form action="submit_headprefect_vote.php" method="POST" id="votingForm">
      <div class="voting-section">
        <div class="position-heading">
          <i class="fas fa-crown"></i>
          HEAD PREFECT
          <?php if ($isSingle): ?>— Sole Candidate<?php else: ?>— All Candidates<?php endif; ?>
        </div>

        <?php if ($isSingle): ?>
          <!-- Single Head Prefect candidate: YES / NO layout -->
          <div class="candidate-container" style="max-width:500px; margin:0 auto;">
            <?php $candidate = $candidates[0];
              $genderLower = strtolower($candidate['gender']);
              $isMale = in_array($genderLower, ['m','male','boy']);
              $genderClass = $isMale ? 'male' : 'female';
              $genderLabel = $isMale ? 'Male Candidate' : 'Female Candidate';
              $genderIcon = $isMale ? 'fa-mars' : 'fa-venus';
            ?>
            <div class="candidate-card" data-position="Head Prefect">
              ...
              <div class="candidate-info">
                <div>
                  ...
                  <p class="candidate-gender <?php echo $genderClass; ?>">
                    <i class="fas <?php echo $genderIcon; ?>"></i> <?php echo $genderLabel; ?>
                  </p>
                ...
                <div>
                  ...
                  <div class="yes-no-group">
                    <input type="radio" class="yes-no-radio yes" name="yesno_vote" value="1" id="hp_yes">
                    <label for="hp_yes" class="yes-btn"><i class="fas fa-thumbs-up"></i> YES</label>
                    <input type="radio" class="yes-no-radio no" name="yesno_vote" value="0" id="hp_no">
                    <label for="hp_no" class="no-btn"><i class="fas fa-thumbs-down"></i> NO</label>
                  </div>
                </div>
              </div>
            </div>
        ...
        <?php else: ?>
          <!-- Multiple Head Prefect candidates: Select layout -->
      ...
      <div class="text-center mt-4">
        <button type="submit" class="vote-btn"><i class="fas fa-check-circle"></i> Submit Your Vote</button>
        <p class="text-muted mt-3">
          <small><i class="fas fa-info-circle"></i>
            <?php if ($isSingle): ?>
              Click <strong>YES</strong> to support or <strong>NO</strong> to oppose the candidate.
            <?php else: ?>
              Click a candidate to select. Click again to deselect.
            <?php endif; ?>
            You may skip this position.
          </small>
        </p>
      </div>
    </form>
...
<div class="footer">
  ...
  <p>Electronic Voting System - Head Prefect Section</p>
</div>
...
$(document).ready(function() {
  ...
  // Multi-candidate deselection
  var lastChecked = null;
  $(".custom-radio").click(function() {
    if (lastChecked === this) {
      $(this).prop('checked', false); lastChecked = null;
  ...
  $("#votingForm").submit(function(e) {
    if (!confirm("Are you sure you want to submit your vote for Head Prefect? This action cannot be undone.")) { e.preventDefault(); return false; }
  });
