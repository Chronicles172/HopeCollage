...
switch ($action) {
    ...
    // ── Single student by ID (lightweight lookup) ─────────────
    case 'student':
        $id = (int)($_GET['id'] ?? 0);
        ...
        $stmt = getDB()->prepare(
            'SELECT id, first_name, last_name, student_class, house,
                    nhis_id, date_of_birth, gender, student_id_no, photo_path
             FROM students
             WHERE id = ?
             LIMIT 1'
        );
        ...
        echo json_encode(['success' => true, 'message' => 'Student loaded.', 'data' => $student]);
        break;
    ...
    case 'student_full_data':
        ...
        // Fetch student — includes medical_condition
        $stmt = $pdb->prepare(
            'SELECT s.id, s.first_name, s.last_name, s.student_class, s.student_id_no,
             ...
             FROM students s
             WHERE s.id = ? AND s.is_linked_copy = 0 LIMIT 1'
        );
        $stmt->execute([$sid]);
        $student = $stmt->fetch();
