...
// Fetch Boys candidates grouped by position
$boysResult = mysqli_query($connect, "SELECT * FROM candidatedetails WHERE gender IN ('M', 'male', 'boy', 'Male', 'Boy') AND LOWER(TRIM(position)) != 'head prefect' ORDER BY position, candidateid");

$positions = [];
while ($row = mysqli_fetch_assoc($boysResult)) {
...
<html>
<head>
<title>Boys Voting Page | Sonrise Christian High School</title>
...
.candidate-card.yes-selected {
  border: 3px solid #28a745;
  box-shadow: 0 0 20px rgba(40, 167, 69, 0.4);
  transform: translateY(-5px);
}
.candidate-card.no-selected {
...
/* Standard select button (multiple candidates) */
.select-btn {
  display: block;
  width: 100%;
...
.custom-radio:checked + .select-btn {
...
/* Yes / No buttons (single candidate) */
.yes-no-group {
  display: flex;
  gap: 10px;
  margin-top: 10px;
...
.yes-no-radio.yes:checked + .yes-btn {
  background: linear-gradient(135deg, #2e7d32, #388e3c);
  color: white;
  border-color: #1b5e20;
}
.yes-no-radio.yes:checked + .yes-btn:after { content: " ✓"; }
.yes-no-radio.no:checked + .no-btn {
...
@media screen and (max-width: 992px) {
  .candidate-container { grid-template-columns: repeat(2, 1fr); gap: 20px; }
  .candidate-image { height: 300px !important; }
  .main { padding: 20px; }
}
@media screen and (max-width: 768px) {
  .sidenav { width: 70px; }
  .sidenav a { padding: 15px 10px; text-align: center; }
  .sidenav a i { margin-right: 0; font-size: 20px; }
  .sidenav a span { display: none; }
  .brand-section h4 { display: none; }
  .main { margin-left: 70px; padding: 15px; }
  .footer { left: 70px; width: calc(100% - 70px); padding: 10px; }
  .toggle-btn { left: 70px; }
  .header h1 { font-size: 1.8rem; }
  .header p { font-size: 1rem; }
  .candidate-container { grid-template-columns: 1fr; gap: 20px; }
  .candidate-image { height: 350px !important; }
  .position-heading { font-size: 1.3rem; padding: 12px 15px; }
  .voting-section { padding: 20px; }
}
@media screen and (max-width: 576px) {
  .header { padding: 20px 15px; }
  .header h1 { font-size: 1.5rem; }
  .main { padding: 10px; }
  .voting-section { padding: 15px; }
  .candidate-image { height: 320px !important; }
  .candidate-info h3 { font-size: 1.1rem; }
  .vote-btn { padding: 12px 30px; font-size: 1rem; }
  .position-heading { font-size: 1.1rem; padding: 10px 12px; }
}
@media screen and (max-width: 400px) {
  .sidenav { width: 60px; }
  .main { margin-left: 60px; }
  .footer { left: 60px; width: calc(100% - 60px); font-size: 12px; }
  .toggle-btn { left: 60px; width: 35px; height: 35px; }
  .candidate-image { height: 280px !important; }
}
@media print {
  .sidenav, .toggle-btn, .footer, .vote-btn { display: none; }
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header bg-danger text-white">
            <h5 class="modal-title" id="errorModalLabel"><i class="fas fa-exclamation-triangle mr-2"></i>Authentication Error</h5>
...
<div class="main">
  <div class="header">
    ...
    <p>Boys' Electronic Voting System</p>
  </div>

  <?php if (empty($positions)): ?>
    <div class="alert alert-info text-center">
      <i class="fas fa-info-circle mr-2"></i>
      <strong>No male candidates available at this time.</strong>
    </div>
  <?php else: ?>
    <form action="submit_boys_vote.php" method="POST" id="votingForm">
      ...
      ?>
              ...
              <div class="candidate-card" data-position="<?php echo htmlspecialchars($position); ?>">
                ...
                <div class="candidate-info">
                  <div>
                    ...
                    <p class="candidate-gender"><i class="fas fa-mars"></i> Male Candidate</p>
                    <?php if ($isSingle): ?>
                  ...
                  <?php if ($isSingle): ?>
                    <!-- YES / NO for single candidate -->
                    <div>
                      <!-- Hidden field carries candidateid so submit script knows who the candidate is -->
                  ...
                  <?php else: ?>
                    <!-- SELECT CANDIDATE for multiple candidates -->
                    <div>
                      <input type="radio"
                             class="custom-radio"
        ...
        <?php if ($position !== array_key_last($positions)): ?>
      ...
      <div class="text-center mt-4">
        <button type="submit" class="vote-btn">
          <i class="fas fa-check-circle"></i> Submit Your Votes
        </button>
...
<div class="footer">
  ...
  <p>Electronic Voting System - Boys' Section</p>
</div>
...
$(document).ready(function() {
  ...
  $("#toggleSidebar").click(function() {
    ...
    if ($(".sidenav").hasClass("collapsed")) {
      ...
      $(".footer").css({"width": "calc(100% - 70px)", "left": "70px"});
      $(".toggle-btn").css("left", "70px");
      $(".sidenav a span, .brand-section h4").hide();
      $(".sidenav a").css({"text-align": "center", "padding": "15px 10px"});
      $(".sidenav a i").css("margin-right", "0");
    } else {
      $(".sidenav").css("width", "220px");
      $(".main").css("margin-left", "220px");
      $(".footer").css({"width": "calc(100% - 220px)", "left": "220px"});
      $(".toggle-btn").css("left", "220px");
      $(".sidenav a span, .brand-section h4").show();
      $(".sidenav a").css({"text-align": "left", "padding": "15px 20px"});
      $(".sidenav a i").css("margin-right", "10px");
    }
  });

  // Multi-candidate: track last checked for deselection support
  var lastChecked = {};
  ...
  // Single-candidate YES/NO: visual highlight
  $(".yes-no-radio.yes").change(function() {
    var card = $(this).closest('.candidate-card');
    card.removeClass('no-selected').addClass('yes-selected');
  });
  $(".yes-no-radio.no").change(function() {
    var card = $(this).closest('.candidate-card');
    card.removeClass('yes-selected').addClass('no-selected');
  });
  ...
  // Hover effect for multi-candidate cards
  $(".candidate-card").hover(
