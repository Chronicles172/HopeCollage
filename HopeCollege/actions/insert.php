...
if (isset($_POST['students']) && is_array($_POST['students'])) {
    ...
    // ──────────────────────────────────────────────────────────
    // 10. ADD ANNOUNCEMENT (admin only)
    // ──────────────────────────────────────────────────────────
    case 'add_announcement':
        ...
        $isPinned = (int)(!empty($_POST['ann_pinned']) && $_POST['ann_pinned'] == '1');

        if (!$title) jsonResponse(false, 'Announcement title is required.');
        if (!$body)  jsonResponse(false, 'Announcement body is required.');

        $stmt = getDB()->prepare(
            'INSERT INTO announcements (title, body, is_pinned, created_by) VALUES (?, ?, ?, ?)'
        );
        $stmt->execute([$title, $body, $isPinned, (int)$_SESSION['admin_id']]);
        jsonResponse(true, 'Announcement published.', ['id' => (int)getDB()->lastInsertId()]);
        break;

    // ──────────────────────────────────────────────────────────
    // 11. DELETE ANNOUNCEMENT (admin only)
    // ──────────────────────────────────────────────────────────
    case 'delete_announcement':
        ...
        $stmt->execute([$annId]);
        if ($stmt->rowCount() === 0) jsonResponse(false, 'Announcement not found.');
    ...
    case 'pin_announcement':
        ...
        getDB()->prepare('UPDATE announcements SET is_pinned = ? WHERE id = ?')
               ->execute([$isPinned ? 1 : 0, $annId]);
        jsonResponse(true, $isPinned ? 'Pinned.' : 'Unpinned.');
        break;
    ...
    case 'update_student':
        ...
        if (!$firstName) jsonResponse(false, 'First name is required.');
        if (!$lastName)  jsonResponse(false, 'Last name is required.');
        if (!$class)     jsonResponse(false, 'Class is required.');
        if (!$gender)    jsonResponse(false, 'Gender is required.');

        $pdb = getDB();
        $stmt = $pdb->prepare(
            'UPDATE students SET
               first_name=?, last_name=?, student_class=?, house=?,
               nhis_id=?, date_of_birth=?, gender=?, medical_condition=?
             WHERE id=? AND is_linked_copy=0'
        );
        $stmt->execute([
            $firstName, $lastName, $class,
            $house ?: null, $nhis ?: null,
            $dob   ?: null, $gender,
            $medical ?: null,
            $id
        ]);
        if ($stmt->rowCount() === 0) jsonResponse(false, 'Student not found or no changes made.');
        jsonResponse(true, 'Student updated successfully.');
        break;

    // ──────────────────────────────────────────────────────────
    // 14. UPDATE PARENT (admin only)
    // ──────────────────────────────────────────────────────────
    case 'update_parent':
        session_start();
        if (empty($_SESSION['admin_id'])) jsonResponse(false, 'Unauthorised.');
        ...
        if (!$id)           jsonResponse(false, 'Parent ID required.');
        if (!$firstName)    jsonResponse(false, 'First name is required.');
        if (!$lastName)     jsonResponse(false, 'Last name is required.');
        if (!$phone)        jsonResponse(false, 'Phone number is required.');
        if (!$relationship) jsonResponse(false, 'Relationship is required.');

        $pdb = getDB();
        $pdb->prepare(
            'UPDATE parents SET
               first_name=?, last_name=?, phone=?, email=?, address=?, relationship=?
             WHERE id=?'
        )->execute([
            $firstName, $lastName, $phone,
            $email ?: null, $address ?: null, $relationship,
            $id
        ]);
        jsonResponse(true, 'Parent updated successfully.');
        break;
