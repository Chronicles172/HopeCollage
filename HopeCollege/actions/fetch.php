...
switch ($action) {
    ...
    case 'parent_by_phone':
        ...
        $ws = $pdb->prepare(
            'SELECT s.id, s.first_name, s.last_name, s.student_class, s.house,
                    s.nhis_id, s.gender, s.photo_path
             FROM students s
             JOIN student_parents sp ON sp.student_id = s.id
    ...
    case 'student_full_data':
        ...
        // Fetch student
        $stmt = $pdb->prepare(
            'SELECT s.id, s.first_name, s.last_name, s.student_class,
    ...
    // ── Parent Portal: all data for one parent ────────────────
    case 'portal_data':
        $parentId = (int)($_GET['parent_id'] ?? 0);
        if (!$parentId) { echo json_encode(['success' => false, 'message' => 'parent_id required']); break; }

        $pdb = getDB();

        // Verify parent exists
        $chk = $pdb->prepare('SELECT id FROM parents WHERE id = ? LIMIT 1');
        $chk->execute([$parentId]);
        if (!$chk->fetch()) { echo json_encode(['success' => false, 'message' => 'Parent not found.']); break; }

        // Wards linked to this parent
        $stmtWards = $pdb->prepare(
            'SELECT s.id, s.first_name, s.last_name, s.student_class, s.house,
                    s.nhis_id, s.gender, s.date_of_birth, s.photo_path
             FROM students s
             JOIN student_parents sp ON sp.student_id = s.id
             WHERE sp.parent_id = ? AND s.is_linked_copy = 0
             ORDER BY s.first_name ASC'
        );
        $stmtWards->execute([$parentId]);
        $wards = $stmtWards->fetchAll();

        // Exeat requests for wards of this parent
        $wardIds = array_column($wards, 'id');
        $exeats  = [];
        if (!empty($wardIds)) {
            $inPlaceholders = implode(',', array_fill(0, count($wardIds), '?'));
            $stmtEx = $pdb->prepare(
                'SELECT e.id, e.reason, e.departure_date, e.departure_time,
                        e.expected_return, e.actual_return, e.status, e.review_note,
                 ...
                 JOIN students s      ON s.id = e.student_id
                 LEFT JOIN admin_users a ON a.id = e.reviewed_by
                 WHERE e.student_id IN (' . $inPlaceholders . ')
                 ORDER BY e.created_at DESC'
            );
            $stmtEx->execute($wardIds);
            $exeats = $stmtEx->fetchAll();
        }

        // All events
        $events = $pdb->query(
            'SELECT id, name, event_type, event_date, event_time, venue, description
