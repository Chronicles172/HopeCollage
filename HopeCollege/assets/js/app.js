...
function renderWardForms(clearValues) {
...
if (!clearValues) {
  document.querySelectorAll('.ward-block').forEach(function(blk, i) {
    existing[i] = {
      ...
      medical: (blk.querySelector('[data-field="medical"]') || {}).value || '',
    };
  });
}
  container.innerHTML = '';
      ...
      '<div class="ward-block" id="ward-block-' + i + '">' +
        ...
        '<div class="form-row">' +
          '<div class="form-group"><label>Class <span class="req">*</span></label>' +
            '<select data-ward="' + i + '" data-field="class">' +
              '<option value="">&#8212; Select Class &#8212;</option>' +
              '<option value="SHS 1"' + (prev.cls==='SHS 1' ?' selected':'') + '>SHS 1</option>' +
              '<option value="SHS 2"' + (prev.cls==='SHS 2' ?' selected':'') + '>SHS 2</option>' +
              '<option value="SHS 3"' + (prev.cls==='SHS 3' ?' selected':'') + '>SHS 3</option>' +
            '</select></div>' +
        ...
        '<div class="form-row single">' +
          '<div class="form-group"><label>NHIS ID Number</label>' +
            '<input type="text" data-ward="' + i + '" data-field="nhis" placeholder="" value="' + (prev.nhis||'') + '"/></div>' +
        '</div>' +
        '<div class="form-row">' +
        ...
        '</div>' +
        '<div class="form-row single">' +
          '<div class="form-group"><label>Medical Condition / Allergies <span style="font-size:11px;color:var(--text-muted)">(Input "No Condition" If there is no Condition)</span></label>' +
            '<textarea data-ward="' + i + '" data-field="medical" placeholder="e.g. Asthma, allergic to peanuts, etc." rows="2" style="width:100%;padding:.65rem;border:1.5px solid var(--border);border-radius:8px;font-family:inherit;font-size:14px;resize:vertical">' + (prev.medical||'') + '</textarea>' +
          '</div>' +
        '</div>' +
        '<div class="form-row single">' +
          '<div class="form-group"><label>Student Photo (Compulsory)</label>' +
  ...
  for (var i = 0; i < wardCount; i++) {
    ...
    container.insertAdjacentHTML('beforeend',
      '<div class="ward-block" id="ward-block-' + i + '">' +
        ...
        '</div>' +
        '<div class="form-row single">' +
          '<div class="form-group"><label>Medical Condition / Allergies <span style="font-size:11px;color:var(--text-muted)">(Optional)</span></label>' +
            '<textarea data-ward="' + i + '" data-field="medical" placeholder="e.g. Asthma, allergic to peanuts, etc." rows="2" style="width:100%;padding:.65rem;border:1.5px solid var(--border);border-radius:8px;font-family:inherit;font-size:14px;resize:vertical">' + (prev.medical||'') + '</textarea>' +
          '</div>' +
        '</div>' +
        '<div class="form-row single">' +
...
function submitNewWards() {
  ...
  for (var i = 0; i < wardCount; i++) {
    ...
    wards.push({
      ...
      medical: ((document.querySelector('[data-ward="' + i + '"][data-field="medical"]') || {}).value || '').trim(),
  ...
  wards.forEach(function(w, i) {
    ...
    fd.append('students[' + i + '][medical]', w.medical);
...
function clearRegForm() {
  ...
  // Clear second parent fields FIRST
toggleSpouseSection(false);

// THEN reset wards
wardCount = 1;

var wcd = document.getElementById('wardCountDisplay');
if (wcd) wcd.textContent = '1';

// Completely rebuild empty forms
document.getElementById('wardFormsContainer').innerHTML = '';

renderWardForms(true);
}
...
function loadAdminAnnouncements() {
  apiFetch('actions/fetch.php?action=announcements').then(function(r) {
    if (!r.success) return;
    _adminAnns = r.data;
...
function renderAdminAnnouncements() {
  var el = document.getElementById('adminAnnList');
  if (!el) return;
  if (!_adminAnns.length) {
    el.innerHTML = '<div style="text-align:center;padding:3rem 0;color:var(--text-muted)"><div style="font-size:2.5rem;margin-bottom:.5rem">📢</div><p>No announcements yet. Create one above.</p></div>';
    return;
  }
  el.innerHTML = _adminAnns.map(function(a) {
    var pinLabel = a.is_pinned == 1
      ? '<span style="background:rgba(212,153,58,.15);color:var(--gold);font-size:10px;font-weight:700;padding:2px 8px;border-radius:10px;letter-spacing:.05em;text-transform:uppercase">📌 Pinned</span>'
      : '';
    var bodyHtml = (a.body||'').replace(/\n/g, '<br/>').replace(/</g,'&lt;').replace(/>/g,'&gt;');
    return '<div style="background:var(--white);border:1px solid var(--border);border-radius:var(--radius);padding:1.25rem 1.5rem;margin-bottom:12px;position:relative">' +
      '<div style="display:flex;align-items:flex-start;justify-content:space-between;gap:1rem;flex-wrap:wrap;margin-bottom:.75rem">' +
        '<div style="min-width:0">' +
          '<div style="font-family:var(--font-display);font-size:1.1rem;color:var(--navy);margin-bottom:4px">' + escAdminHtml(a.title) + ' ' + pinLabel + '</div>' +
          '<div style="font-size:12px;color:var(--text-muted)">' + fmtDatetime(a.created_at) + (a.author_name ? ' &bull; ' + escAdminHtml(a.author_name) : '') + '</div>' +
        '</div>' +
        '<div style="display:flex;gap:8px;flex-shrink:0">' +
          '<button class="' + (a.is_pinned==1?'btn-primary':'btn-outline') + '" style="font-size:12px;padding:5px 12px;' + (a.is_pinned!=1?'color:var(--navy);border-color:var(--border)':'') + '" onclick="togglePin(' + a.id + ',' + (a.is_pinned==1?0:1) + ')">' + (a.is_pinned==1?'📌 Unpin':'📌 Pin') + '</button>' +
          '<button class="btn-danger" style="font-size:12px;padding:5px 12px" onclick="deleteAnn(' + a.id + ')">Delete</button>' +
        '</div>' +
      '</div>' +
      '<div style="font-size:14px;color:var(--text);line-height:1.6;background:var(--cream);border-radius:8px;padding:.75rem 1rem">' + bodyHtml + '</div>' +
    '</div>';
  }).join('');
}
...
function saveAnnouncement() {
  ...
  var pinned = document.getElementById('annPinned').checked ? '1' : '0';
  if (!title) { showToast('Title is required.', 'error'); return; }
  if (!body)  { showToast('Announcement body is required.', 'error'); return; }
  var fd = new FormData();
  fd.append('action',      'add_announcement');
  fd.append('ann_title',   title);
  fd.append('ann_body',    body);
  fd.append('ann_pinned',  pinned);
  apiPost('actions/insert.php', fd).then(function(r) {
    if (r.success) {
...
function deleteAnn(id) {
  ...
  fd.append('ann_id', id);
  apiPost('actions/insert.php', fd).then(function(r) {
    if (r.success) { showToast('Deleted.', 'success'); loadAdminAnnouncements(); }
    else           { showToast(r.message, 'error'); }
  });
}

function togglePin(id, newPin) {
  var fd = new FormData();
  fd.append('action',    'pin_announcement');
  fd.append('ann_id',    id);
  fd.append('is_pinned', newPin);
  apiPost('actions/insert.php', fd).then(function(r) {
    if (r.success) { loadAdminAnnouncements(); }
    else           { showToast(r.message, 'error'); }
  });
}
...
function initAdminSearchDropdown() {
  ...
  input.addEventListener('input', function() {
    ...
    if (!results) return;
    // Also filter the table immediately
    renderParentsTable();
    ...
    _adminSearchTimer = setTimeout(function() {
      apiFetch('actions/fetch.php?action=search_parents&q=' + encodeURIComponent(q)).then(function(r) {
        ...
        r.data.forEach(function(row) {
          ...
          var gIcon = row.gender === 'Male' ? '👨‍🎓' : row.gender === 'Female' ? '👩‍🎓' : '🎓';
          item.innerHTML =
            '<div style="width:32px;height:32px;border-radius:50%;background:var(--navy);color:var(--gold);display:flex;align-items:center;justify-content:center;font-size:.7rem;font-weight:700;flex-shrink:0">' + gIcon + '</div>' +
            '<div style="flex:1;min-width:0">' +
              '<div class="s-name">' + row.s_first + ' ' + row.s_last + '</div>' +
              '<div class="s-meta">' + (row.student_class||'') + '</div>' +
              '<div class="s-meta" style="color:var(--navy-light)">👥 ' + row.p_first + ' ' + row.p_last + ' <span style="opacity:.7">(' + row.relationship + ')</span></div>' +
            '</div>';
          item.addEventListener('pointerdown', function(e) {
            e.preventDefault();
            var inp = document.getElementById('searchParents');
            if (inp) { inp.value = row.s_first + ' ' + row.s_last; renderParentsTable(); }
  ...
  input.addEventListener('keydown', function(e) {
    if (e.key === 'Escape') {
      var r = document.getElementById('adminSearchResults');
      if (r) { r.innerHTML = ''; r.classList.remove('open'); }
    }
  });
}

function initHpSearchDropdown() {
  var input = document.getElementById('hpStudentSearch');
  if (!input || input.dataset.liveSearch) return;
  input.dataset.liveSearch = '1';
  ...
  if (!input.parentElement.classList.contains('search-box')) {
    var box = document.createElement('div');
    box.className = 'search-box';
    box.style.cssText = 'position:relative;flex:1;min-width:180px';
    input.parentNode.insertBefore(box, input);
    box.appendChild(input);
    input.style.width = '100%';
    input.style.minWidth = '';
    var dd = document.createElement('div');
    dd.className = 'search-results';
    dd.id = 'hpSearchResults';
    box.appendChild(dd);
  }

  input.addEventListener('input', function() {
    var q = this.value.trim();
    clearTimeout(_hpSearchTimer);
    var results = document.getElementById('hpSearchResults');
    if (!results) return;
    renderHpStudents();
    if (q.length < 2) { results.innerHTML = ''; results.classList.remove('open'); return; }
    _hpSearchTimer = setTimeout(function() {
      apiFetch('actions/fetch.php?action=search_parents&q=' + encodeURIComponent(q)).then(function(r) {
        results.innerHTML = '';
        if (!r.success || !r.data.length) { results.classList.remove('open'); return; }
        // filter to current gender
        var hits = r.data.filter(function(s) { return !hpGender || s.gender === hpGender || !s.gender; });
        if (!hits.length) { results.classList.remove('open'); return; }
        // dedupe by student id
        var seen = {};
        hits = hits.filter(function(s) { if (seen[s.s_id]) return false; seen[s.s_id]=1; return true; });
        hits.forEach(function(s) {
          ...
          item.innerHTML =
            ...
            '<div style="flex:1;min-width:0">' +
              ...
              '<div class="s-meta">' + (s.student_class||'') + '</div>' +
              '<div class="s-meta" style="color:var(--navy-light)">👥 ' + s.p_first + ' ' + s.p_last + ' <span style="opacity:.7">(' + s.relationship + ')</span></div>' +
            '</div>';
          item.addEventListener('pointerdown', function(e) {
            e.preventDefault();
            var inp = document.getElementById('hpStudentSearch');
            if (inp) { inp.value = s.s_first + ' ' + s.s_last; renderHpStudents(); }
            results.innerHTML = ''; results.classList.remove('open');
          });
          results.appendChild(item);
        });
...
function renderParentsTable() {
  ...
  var gender = (document.getElementById('filterGender')  || {value:''}).value;

  // Filter by relationship only at the parent level (rel doesn't affect which student shows)
  var filtered = allParents.filter(function(p) {
    return !rel || p.relationship === rel;
  });

  // Build student groups first, then filter groups by the student's own class/search
  var result = buildStudentGroups(filtered);

  // Filter groups: class, gender and search operate on the STUDENT row, not parent wards
  result.groups = result.groups.filter(function(group) {
    var w = group.student;

    // Class filter from dropdown — exact match
    if (cls && w.student_class !== cls) return false;

    // Gender filter — exact match
    if (gender && w.gender !== gender) return false;

    // Free-text search — match name, ID, parent info as substrings
    // but match class as a whole word to avoid "2" hitting "SHS 12" or "SHS 2A"
    if (q) {
      var nonClassText = (
        w.first_name + ' ' + w.last_name + ' ' +
        group.parents.map(function(p) {
          return p.first_name + ' ' + p.last_name + ' ' + p.phone;
        }).join(' ')
      ).toLowerCase();

      var classText = (w.student_class || '').toLowerCase();

      // Whole-word match against class: e.g. "2" matches "shs 2" but not "shs 2a" or "shs 12"
      var classMatch = classText
        ? new RegExp('(?:^|\\s)' + q.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '(?:\\s|$)').test(classText)
        : false;

      var nameMatch = nonClassText.indexOf(q) !== -1;

      if (!classMatch && !nameMatch) return false;
    }

    return true;
  });

  // Also filter unlinked parents by search/rel
  result.unlinked = result.unlinked.filter(function(p) {
    var searchText = (p.first_name+' '+p.last_name+' '+p.phone).toLowerCase();
    return (!q || searchText.indexOf(q) !== -1) && (!rel || p.relationship === rel);
  });

  var tbody = document.getElementById('parentsTableBody'); if (!tbody) return;
  if (!result.groups.length && !result.unlinked.length) {
    tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;padding:2rem;color:var(--text-muted)">No records found.</td></tr>';
    return;
  }
  var rows = '';

  // ── Group student rows by gender: Male → Female → Other ──
  var genderOrder = ['Male', 'Female', 'Other'];
  var genderGroups = { Male: [], Female: [], Other: [] };
  result.groups.forEach(function(group) {
    var g = (group.student.gender || 'Other');
    if (!genderGroups[g]) g = 'Other';
    genderGroups[g].push(group);
  });

  // Count how many gender buckets have entries (to hide header when only one gender is shown)
  var activeBuckets = genderOrder.filter(function(g) { return genderGroups[g] && genderGroups[g].length; }).length;

  genderOrder.forEach(function(g) {
    if (!genderGroups[g] || !genderGroups[g].length) return;
    // Only show gender section header when there are multiple genders visible
    if (activeBuckets > 1) {
      var icon = g === 'Male' ? '👨‍🎓' : g === 'Female' ? '👩‍🎓' : '🎓';
      rows += '<tr><td colspan="7" style="background:var(--cream);padding:8px 14px;font-size:.72rem;' +
              'font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--navy-mid);border-bottom:2px solid var(--border)">' +
              icon + '&ensp;' + g + ' Students&ensp;<span style="font-weight:400;color:var(--text-muted)">(' + genderGroups[g].length + ')</span></td></tr>';
    }

    // ── One row per student, listing all their parents ────────
    genderGroups[g].forEach(function(group) {
    var w  = group.student;
    var wa = w.photo_path
    ...
    var parentNames = group.parents.map(function(p) {
      return '<span style="display:inline-flex;align-items:center;gap:5px;background:var(--cream);border:1px solid var(--border);border-radius:20px;padding:2px 10px;font-size:12px;margin:2px">' +
        ...
        '<button onclick="openEditParent(' + p.id + ')" style="background:none;border:none;color:var(--gold);cursor:pointer;font-size:11px;padding:0 2px;opacity:.8" title="Edit">✏️</button>' +
      '</span>';
    }).join('');

    rows += '<tr>' +
      '<td><div class="parent-cell">' + wa + '<div><div style="font-weight:600">' + w.first_name + ' ' + w.last_name + '</div></div></div></td>' +
      ...
      '<td style="white-space:nowrap">' +
        '<button onclick="viewStudent(' + w.id + ')" style="background:var(--navy);border:none;color:white;cursor:pointer;font-size:11px;padding:4px 10px;border-radius:4px;margin-right:4px" title="View Student Details">👁 View</button>' +
        '<button onclick="openEditStudent(' + w.id + ')" style="background:var(--gold);border:none;color:var(--navy);cursor:pointer;font-size:11px;padding:4px 10px;border-radius:4px;font-weight:600" title="Edit Student">✏️ Edit</button>' +
      '</td>' +
    '</tr>';
    }); // end genderGroups[g].forEach
  }); // end genderOrder.forEach
...
function viewStudent(studentId) {
  // Find the student in allParents data
  var student = null;
  var studentParents = [];
  
  for (var i = 0; i < allParents.length; i++) {
    var p = allParents[i];
    if (p.wards && p.wards.length) {
      for (var j = 0; j < p.wards.length; j++) {
        if (p.wards[j].id === studentId) {
          student = p.wards[j];
          studentParents.push(p);
        }
      }
    }
  }
  ...
  '<div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin-bottom:1.5rem">' +
    ...
    '<div><div style="font-size:11px;font-weight:700;color:var(--text-muted);text-transform:uppercase;margin-bottom:4px;letter-spacing:.05em">DATE OF BIRTH</div>' +
      '<div style="font-size:15px;font-weight:600">' + (student.date_of_birth || 'Not specified') + '</div></div>' +
  ...
  '</div>' +
  (student.medical_condition ? '<div style="background:rgba(255,193,7,.1);border:1.5px solid rgba(255,193,7,.4);border-radius:10px;padding:1rem;margin-bottom:1.5rem">' +
    '<div style="font-size:11px;font-weight:700;color:var(--text-muted);text-transform:uppercase;margin-bottom:6px;letter-spacing:.05em;display:flex;align-items:center;gap:6px"><span style="font-size:16px">⚕️</span> MEDICAL CONDITION / ALLERGIES</div>' +
    '<div style="font-size:14px;line-height:1.5;color:var(--navy)">' + student.medical_condition + '</div>' +
  '</div>' : '') +
...
// ── GPS CAMPUS CHECK ──────────────────────────────────────────
var SCHOOL_LAT      =  7.403389;
var SCHOOL_LNG      =  0.474083;
var SCHOOL_RADIUS_M =  150;   // tighter radius — accurate readings make this reliable
...
function checkGPS() {
  ...
  var blocked = document.getElementById('gpsBlockedMsg');
  var allowed = document.getElementById('gpsAllowedForm');
  if (!banner) return;

  banner.className = 'gps-banner checking';
  banner.innerHTML = '<span class="gps-icon">&#128225;</span><div><b>Acquiring location…</b> Sampling GPS for best accuracy — this takes a few seconds.</div>';
  ...
  // Collect up to MAX_SAMPLES readings over SAMPLE_WINDOW ms.
  // Keep only readings whose accuracy <= MAX_ACCURACY_M (GPS-quality).
  // Use the reading with the best (lowest) accuracy value.
  // If no high-accuracy reading arrives in time, fall back to the best we got.

  var MAX_SAMPLES    = 5;
  var SAMPLE_WINDOW  = 12000;   // 12 seconds total
  var MAX_ACCURACY_M = 50;      // discard readings worse than 50 m accuracy
  ...
  function applyResult(best) {
    ...
    var dist = Math.round(getDistanceMetres(best.latitude, best.longitude, SCHOOL_LAT, SCHOOL_LNG));
    var accLabel = Math.round(best.accuracy) + 'm accuracy';

    // Effective radius = school radius + GPS accuracy margin (but capped at 30 m extra)
    var margin = Math.min(Math.round(best.accuracy * 0.5), 30);
    var effectiveRadius = SCHOOL_RADIUS_M + margin;

    if (dist <= effectiveRadius) {
      banner.className = 'gps-banner allowed';
      banner.innerHTML = '<span class="gps-icon">&#10003;</span><div><b>You are on campus</b> ' +
        dist + 'm from school &nbsp;·&nbsp; <span style="font-size:12px;opacity:.8">' + accLabel + '</span></div>';
      if (allowed) allowed.style.display = 'block';
      if (blocked) blocked.style.display = 'none';
    } else {
      banner.className = 'gps-banner blocked';
      banner.innerHTML = '<span class="gps-icon">&#128683;</span><div><b>Not on campus</b> ' +
        'You are ' + dist + 'm away &nbsp;·&nbsp; <span style="font-size:12px;opacity:.8">' + accLabel + '</span>' +
        '<br/><span style="font-size:12px;opacity:.75">Must be within ' + SCHOOL_RADIUS_M + 'm of campus.</span></div>';
      if (allowed) allowed.style.display = 'none';
      if (blocked) blocked.style.display = 'block';
    }
  }
  ...
  function onSample(pos) {
    if (finished) return;
    var c = pos.coords;
    samples.push({ latitude: c.latitude, longitude: c.longitude, accuracy: c.accuracy });

    // Update banner with live progress
    var pct = Math.round((samples.length / MAX_SAMPLES) * 100);
    var accNow = Math.round(c.accuracy);
    banner.innerHTML = '<span class="gps-icon">&#128225;</span><div><b>Locking on…</b> ' +
      'Sample ' + samples.length + '/' + MAX_SAMPLES + ' &nbsp;·&nbsp; accuracy: ' + accNow + 'm</div>';

    // If we have a GPS-quality reading, stop early
    if (c.accuracy <= MAX_ACCURACY_M && samples.length >= 2) {
      // Pick best accuracy from collected so far
      var best = samples.reduce(function(a, b) { return a.accuracy <= b.accuracy ? a : b; });
      applyResult(best);
      return;
    }

    // All samples collected — use best available
  ...
  function onError(err) {
    if (finished) return;
    // If we already have some samples, use the best one
    if (samples.length > 0) {
      var best = samples.reduce(function(a, b) { return a.accuracy <= b.accuracy ? a : b; });
      applyResult(best);
      return;
    ...
    if (err.code === 2) msg = 'Location unavailable. Move to an open area and try again.';
    if (err.code === 3) msg = 'Location timed out. Move to an open area and try again.';
    banner.innerHTML = '<span class="gps-icon">&#9888;</span><div><b>Location Error</b> ' + msg +
      ' <button onclick="checkGPS()" style="margin-left:8px;padding:3px 10px;font-size:12px;cursor:pointer;border:1px solid white;border-radius:4px;background:transparent;color:white">Retry</button></div>';
    if (blocked) blocked.style.display = 'block';
  }

  // Watch position (keeps getting updates as GPS warms up)
  ...
  // Hard cutoff — accept best sample we have after SAMPLE_WINDOW
  setTimeout(function() {
    if (finished) return;
    if (samples.length > 0) {
      var best = samples.reduce(function(a, b) { return a.accuracy <= b.accuracy ? a : b; });
...
function renderRegistry(parents) {
  ...
  // ── Render student-grouped cards sorted by gender ──────────
  var _genderOrder = ['Male', 'Female', 'Other'];
  var _gGroups = {};
  Object.values(studentMap).forEach(function(group) {
    var g = group.student.gender || 'Other';
    if (!_gGroups[g]) _gGroups[g] = [];
    _gGroups[g].push(group);
  });

  _genderOrder.forEach(function(g) {
    if (!_gGroups[g] || !_gGroups[g].length) return;
    var gIcon = g === 'Male' ? '👨‍🎓' : g === 'Female' ? '👩‍🎓' : '🎓';
    html += '<div style="font-size:.72rem;font-weight:700;text-transform:uppercase;letter-spacing:.08em;' +
            'color:var(--text-muted);padding:.5rem 0 .4rem;margin-top:1rem;border-bottom:1px solid var(--border);margin-bottom:.75rem">' +
            gIcon + '  ' + g + ' Students (' + _gGroups[g].length + ')</div>';
    _gGroups[g].forEach(function(group) {
    var w = group.student;
    var wa = w.photo_path
    ...
    html +=
      ...
      '</div>';
    }); // end _gGroups[g].forEach
  }); // end _genderOrder.forEach
