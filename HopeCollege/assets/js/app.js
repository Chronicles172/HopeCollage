      ...
      '<div class="ward-block" id="ward-block-' + i + '">' +
        ...
        '<div class="form-row">' +
          '<div class="form-group"><label>Class <span class="req">*</span></label>' +
            '<input type="text" data-ward="' + i + '" data-field="class" placeholder="" value="' + (prev.cls||'') + '"/></div>' +
        ...
        r.data.forEach(function(row) {
          ...
          item.innerHTML =
            ...
            '<div style="flex:1;min-width:0">' +
              ...
              '<div class="s-meta">' + (row.student_class||'') + (row.student_id_no ? ' · ID: '+row.student_id_no : '') + '</div>' +
        ...
        hits.forEach(function(s) {
          ...
          item.innerHTML =
            ...
            '<div style="flex:1;min-width:0">' +
              ...
              '<div class="s-meta">' + (s.student_class||'') + (s.student_id_no ? ' · ID: '+s.student_id_no : '') + '</div>' +
...
function renderParentsTable() {
  ...
  result.groups = result.groups.filter(function(group) {
    var w = group.student;
    var searchText = (w.first_name+' '+w.last_name+' '+(w.student_class||'')+(w.student_id_no||'')+
      group.parents.map(function(p){ return p.first_name+' '+p.last_name+' '+p.phone; }).join(' ')).toLowerCase();
    return (!cls    || w.student_class === cls) &&
           (!gender || w.gender === gender) &&
           (!q      || searchText.indexOf(q) !== -1);
  });
